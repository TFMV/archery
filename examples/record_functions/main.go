...
func main() {
	...
	// Demonstrate filtering using package-level functions
	demonstrateFiltering(record, mem)

	// Demonstrate sorting using package-level functions
	demonstrateSorting(record, mem)

	// Demonstrate aggregation using package-level functions
	demonstrateAggregation(record, mem)

	// Demonstrate grouping using package-level functions
	demonstrateGroupBy(mem)
}
...
func demonstrateFiltering(record arrow.Record, mem memory.Allocator) {
	ctx := context.Background()

	// Filter records where age > 30
	fmt.Println("Filtering records where age > 30:")
	filteredByAge, err := archery.FilterRecordGreaterThan(ctx, record, "age", int64(30), mem)
	if err != nil {
	...
	filteredByScore, err := archery.FilterRecordBetween(ctx, record, "score", 80.0, 90.0, mem)
	if err != nil {
	...
	filteredByCustom, err := archery.FilterRecordByColumn(ctx, record, "id", func(arr arrow.Array, i int) bool {
...
func demonstrateSorting(record arrow.Record, mem memory.Allocator) {
	ctx := context.Background()

	// Sort by age in ascending order
	fmt.Println("Sorting by age (ascending):")
	sortedByAge, err := archery.SortRecordByColumn(ctx, record, "age", archery.Ascending, mem)
	if err != nil {
	...
	sortedByScore, err := archery.SortRecordByColumn(ctx, record, "score", archery.Descending, mem)
	if err != nil {
...
func demonstrateAggregation(record arrow.Record, mem memory.Allocator) {
	ctx := context.Background()

	// Calculate sum of ages
	ageSum, err := archery.SumRecordColumn(ctx, record, "age", mem)
	if err != nil {
	...
	scoreMean, err := archery.MeanRecordColumn(ctx, record, "score", mem)
	if err != nil {
	...
	ageMin, err := archery.MinRecordColumn(ctx, record, "age", mem)
	if err != nil {
		log.Fatalf("Error calculating min age: %v", err)
	}
	fmt.Printf("Min age: %v\n", ageMin)

	ageMax, err := archery.MaxRecordColumn(ctx, record, "age", mem)
	if err != nil {
...
func demonstrateGroupBy(mem memory.Allocator) {
	// Create a schema for our record
	schema := arrow.NewSchema(
		[]arrow.Field{
			{Name: "id", Type: arrow.PrimitiveTypes.Int64},
